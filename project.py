...
#Para um ataque de falsificacao de requisicao, o invasor teria que adivinhar o codigo gerado por essa funcao abaixo para fazer uma requisicao em nome do usuario
@app.route('/login')
def showLogin():
...
def gconnect():
    ...
    code = request.data
    try:
        #tentarei usar esse codigo avulso e troca-lo por um objeto credentials que tenha o token de acesso para meu servidor
        oauth_flow = flow_from_clientsecrets('client_secrets.json', scope='')
        oauth_flow.redirect_uri = 'postmessage'
        credentials = oauth_flow.step2_exchange(code)
    except FlowExchangeError:
        response = make_response(json.dumps('Falha ao atualizar o codigo de autorizacao.'), 401)
        response.headers['Content-Type'] = 'application/json'
        return response
...
def deleteRestaurant(restaurant_id):
    ...
    if restaurantToDelete.user_id != login_session['user_id']:
        return "<script>function myFunction() {alert('Voce nao esta autorizado a deletar este restaurante. Favor criar seu proprio restaurante');}</script><body onload='myFunction()''>"
    if request.method == 'POST':
